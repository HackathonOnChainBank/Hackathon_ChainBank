...
const WithdrawDepositComponent = () => {
  ...
  const handleWithdrawDeposit = async () => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        setSuccess('領取定存成功！');
        console.log('WithdrawDeposit 成功，回傳收據:', receipt);
      }

    } catch (err) {
      console.error('WithdrawDeposit 調用錯誤:', err);
      setError(err.message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="withdraw-deposit-container">
      ...
      >
        {loading ? '處理中...' : '執行 WithdrawDeposit'}
      </button>
    </div>
  );
};

export default WithdrawDepositComponent;
