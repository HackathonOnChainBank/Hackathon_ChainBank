...
const RenounceRoleComponent = () => {
  ...
  const handleRenounceRole = async () => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        setSuccess('放棄成功！');
        console.log('RenounceRole 成功，回傳收據:', receipt);
      }

    } catch (err) {
      console.error('RenounceRole 調用錯誤:', err);
      setError(err.message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="renounce-role-container">
      <h2>RenounceRole 放棄角色</h2>
