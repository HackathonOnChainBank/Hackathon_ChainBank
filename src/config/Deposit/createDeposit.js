...
async function main() {
  ...
  const amount = await ask("輸入定存金額（wei）: ");
  const period = await ask("輸入定存期（秒）: ");
  const interest = await ask("輸入利息（wei）: ");
  rl.close();

  try {
    const tx = await contract.createDeposit(user, amount, period, interest);
    await tx.wait();
