...
async function main() {
    ...
    const amounts = (await ask("輸入發息金額陣列（逗號分隔）: ")).split(",").map(x => x.trim());
    rl.close();
    try {
        const tx = await contract.sendInterestBatch(addresses, amounts);
        await tx.wait();
        console.log("批量活存息空投成功，Tx:", tx.hash);
    } catch (err) {
        console.error("批量活存息發放失敗:", err.reason || err.message);
    }
}
main();
