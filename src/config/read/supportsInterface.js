...
export async function supportsInterface(interfaceId) {
  try {
    ...
    console.error("讀取 supportsInterface 失敗:", err.message);
  }
}

async function main() {
  const interfaceId = await ask("請輸入 interfaceId (bytes4, hex): ");
  const result = await supportsInterface(interfaceId);
  console.log(`supportsInterface(${interfaceId}):`, result);
  rl.close();
}
main();
