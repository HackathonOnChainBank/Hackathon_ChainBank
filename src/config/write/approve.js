...
async function main() {
  const spender = await ask("請輸入 spender 地址: ");
  const amountRaw = await ask("請輸入金額（Ether）: ");
  rl.close();
  const amount = ethers.parseUnits(amountRaw, 18);
  try {
    const tx = await contract.approve(spender, amount);
    console.log("approve 交易送出, hash:", tx.hash);
    const receipt = await tx.wait();
    console.log("approve 完成（區塊高度）:", receipt.blockNumber);
  } catch (err) {
    console.error("approve 執行失敗:", err.message);
  }
}
main();
