...
async function main() {
  ...
  const account = await ask("請輸入被撤銷 address: ");
  rl.close();
  try {
    const tx = await contract.revokeRole(role, account);
    console.log("revokeRole 交易送出, hash:", tx.hash);
    const receipt = await tx.wait();
    console.log("撤銷完成，區塊高度:", receipt.blockNumber);
  } catch (err) {
    console.error("revokeRole 執行失敗:", err.message);
  }
}
main();
