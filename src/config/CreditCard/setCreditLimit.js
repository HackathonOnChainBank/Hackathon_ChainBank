...
async function main() {
  ...
  const amount = await ask("輸入信用卡額度（wei）: ");
  rl.close();

  try {
    const tx = await contract.setCreditLimit(user, amount);
    await tx.wait();
    console.log("信用卡額度設定成功，Tx:", tx.hash);
  } catch (err) {
    console.error("設定失敗:", err.reason || err.message);
  }
}
main();
